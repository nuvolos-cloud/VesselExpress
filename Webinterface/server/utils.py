...
from os import listdir, path, remove as osremove, rmdir, walk
from subprocess import run as sbsrun
            ...
            if dir == current_app.config["UPLOAD_FOLDER"] and len(subdirs) > 0:
                for entry in subdirs:
                    if path.join(dir, entry) not in subdirlist:
                        subdirlist.append(
                            path.join(dir, entry)
                        )  # Saves subdir folder names to delete separately #
            if dir != current_app.config["UPLOAD_FOLDER"] and len(subdirs) > 0:
                for entry in subdirs:
                    if path.join(dir, entry) not in subsubdirlist:
                        subsubdirlist.append(
                            path.join(dir, entry)
                        )  # Saves subsubdir folder names to delete separately #
            for file in files:
                if not file.endswith(
                    ".json"
                ):  # Deletes all files except the config files #
                    osremove(path.join(dir, file))
                    number_of_files += 1
        if (
            len(subdirlist) > 0
        ):  # Delete subdir folders after all files have been counted #
...
def download_images(all_results=None, flash_collection=None, firstrun=None):
    ...
    """
    if not any(
        not (files.endswith((tuple(ALLOWED_EXTENSIONS) + (".json", ".zip"))))
        for files in listdir(current_app.config["UPLOAD_FOLDER"])
    ):
        # If no files other than .json and .zip in folder return this #
        if firstrun == "first_run":
            flash_collection.append(
                {"message": "No results to download", "type": "error"}
            )
            return flash_collection
    else:
        ...
        with ZipFile(DOWNLOAD_RESULTS, "w") as zip_filename:
            ...
            for dir, _, files in walk(current_app.config["UPLOAD_FOLDER"]):
                ...
                zipdir = (
                    dir.replace("VesselExpress/data/", "")
                    if "VesselExpress/data/" in dir
                    else dir.replace("VesselExpress/data", "")
                )
                if (
                    all_results == 1
                ):  # Write all files into the zip file except config_standard and itself (loop) #
                    with open(
                        current_app.config["UPLOAD_FOLDER"] + "/progbar.json", "w"
                    ) as progbar_json:
                        dump({"process": "on"}, progbar_json, indent=2)
                    for filename in files:
                        if not (
                            filename.endswith(".zip")
                            or filename == "config_standard.json"
                            or filename == "progbar.json"
                        ):
                            zip_filename.write(
                                path.join(dir, filename), path.join(zipdir, filename)
                            )
                    osremove(
                        path.join(current_app.config["UPLOAD_FOLDER"], "progbar.json")
                    )
...
def download_logs(flash_collection=None, firstrun=None):
    ...
    if len(list_of_logfiles) > 0:
        if firstrun:
            ...
            with ZipFile(
                path.join(current_app.config["UPLOAD_FOLDER"], DOWNLOAD_LOGS), "w"
            ) as zip_filename:
                [
                    zip_filename.write(
                        logfile,
...
def execute_pipeline(queue):
    ...
    """
    snakefile = path.join(
        current_app.config["VESSELEXPRESS_BASE"], "workflow/Snakefile"
    )
    sbsrun(
        f'snakemake --use-conda --cores all --conda-frontend mamba --snakefile "{snakefile}" --directory "{current_app.config["VESSELEXPRESS_BASE"]}"',
        shell=True,
    )
    queue.put(1)
...
def get_last_logfile():
    ...
    try:  # If log present return it to the console window #
        log_path = path.join(current_app.config["VESSELEXPRESS_BASE"], ".snakemake/log")
        glob_pattern = path.join(log_path, r"*log")
        list_of_logfiles = glob(glob_pattern)  # List of log files #
        latest_logfile = max(
            list_of_logfiles, key=path.getctime
        )  # Get the latest log #
        with open(latest_logfile, "r") as logfile:
            processed_log = "".join([line for line in logfile])[
                :-1
            ]  # [:-1] to align blinking marker behind it #
        ...
        if "VesselExpress/data" in dir:
            ...
            [
                rendered_files.append(
                    [
                        file[:-4]
                        .replace("Binary_", "Binary: ")
                        .replace("Skeleton_", "Skeleton: ")
                    ]
                )
                for file in files
                if file not in rendered_files and file.endswith(".glb")
            ]
...
def page_load():
    ...
    """
    with open(
        path.join(current_app.config["UPLOAD_FOLDER"], "config.json")
    ) as config_json:
        current_config = load(config_json)
    for value, subcategory in zip(
        current_config["graphAnalysis"]["pixel_dimensions"].split(","),
        ["dim_z", "dim_y", "dim_x"],
    ):
...
def reset_config(flash_collection):
    ...
    """
    with open(
        path.join(current_app.config["UPLOAD_FOLDER"], "config_standard.json")
    ) as standard_config:
        standard_config = load(standard_config)
    with open(
        path.join(current_app.config["UPLOAD_FOLDER"], "config.json"), "w"
    ) as config_json:
        dump(standard_config, config_json, indent=2)
    flash_collection.append(
        {"message": "Settings reset successfully", "type": "success"}
    )
    return flash_collection
...
def run_pipeline(flash_collection):
    ...
    """
    if not any(
        (image.endswith(tuple(ALLOWED_EXTENSIONS)))
        for image in listdir(current_app.config["UPLOAD_FOLDER"])
    ):
        flash_collection.append(
            {"message": "Found nothing to process!", "type": "warning"}
        )
        return flash_collection
    else:
        ...
        event, queue = (
            Event(),
            Queue(),
        )  # Event synchronizes Processes and Queue allows communication #
        process1 = Process(target=execute_pipeline, args=(queue,))
        process1.start()
        event.set()
        while is_pipeline_not_done:
        ...
        flash_collection.append(
            {"message": "The pipeline has finished!", "type": "finish"}
        )
        return flash_collection
...
def update_config(form, flash_collection):
    ...
    # Catch special cases #
    input_values["render_device"] = (
        "GPU" if input_values["render_device"] == 1 else "CPU"
    )
    input_values["pixel_dimensions"] = ",".join(
        [
            str(input_values["dim_z"]),
    ...
    for category in current_config:  # Iterate through the config file to update it #
        if category not in SKIP_CATEGORIES:
            if category in {
                "small_RAM_mode",
                "render",
            }:  # First level options from the config file #
                if (
                    current_config[category] != input_values[category]
                ):  # Did the value change? #
                    current_config[category] = input_values[category]  # Apply change #
            else:
                for subcategory in current_config[
                    category
                ]:  # Options with subcategories #
                    if (
                        subcategory in input_values
                    ):  # Second level options from the config file #
                        if (
                            current_config[category][subcategory]
                            != input_values[subcategory]
                        ):  # Change? #
                            current_config[category][subcategory] = input_values[
                                subcategory
                            ]  # Apply change #
    flash_collection.append(
    ...
    with open(
        path.join(current_app.config["UPLOAD_FOLDER"], "config.json"), "w"
    ) as config_json:  # Save the new config #
