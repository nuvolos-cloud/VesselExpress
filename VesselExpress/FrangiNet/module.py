...
def main_FN_test(
    image, o, model, mode, normalization, average, mode_img, gpus, batch_size
):
    ...
    data = DataProvider(
        image, normalization, mode_img, batch_size
    ...
    fraggi = FrangiNet()
    fraggi_logits = fraggi.multi_scale(
        inputs=preprocessed, is_training=data.is_training
    )
    ...
    with tf.Session(config=conf) as sess:
        ...
        if data.mode == "3DCube":
            for index in range(0, data.number_cubes):
                ...
                data.result_image[
                    data.positions[index][0] : data.positions[index][0]
                    + data.cube_size,
                    data.positions[index][1] : data.positions[index][1]
                    + data.cube_size,
                    data.positions[index][2] : data.positions[index][2]
                    + data.cube_size,
                ] = np.stack(
                    [pred_x[:, :, :, -1], rot_y[:, :, :], rot_z[:, :, :]], axis=0
                ).max(
                    axis=0
                )
                ...
                print(
                    "Finished Cube " + str(index + 1) + " of " + str(data.number_cubes)
                )
            ...
            image_y = np.zeros(
                shape=(data.dim_xyz[2], data.dim_xyz[1], data.dim_xyz[0]),
                dtype=np.float32,
            )
            for i in range(0, data.dim_xyz[1]):  # "rotate" y dimension
                image_y[: data.dim_xyz[2], i, : data.dim_xyz[0]] = v_p[
                    i, : data.dim_xyz[2], : data.dim_xyz[0], -1
                ]
            ...
            for i in range(0, data.dim_xyz[0]):
                image_z[: data.dim_xyz[2], : data.dim_xyz[1], i] = v_p[
                    i, : data.dim_xyz[2], : data.dim_xyz[1], -1
                ]
            ...
            if average is True:
                ...
                data.result_image = np.stack([image_x, image_y, image_z], axis=0).max(
                    axis=0
                )
            ...
            for ind in range(
                0,
                data.dim_xyz[2] // data.batch_size
                + (data.dim_xyz[2] % data.batch_size > 0),
            ):
                x_test, m_test = data.provide_test_onecube_batch("x")
                feed_dict = {data.x: x_test, data.m: m_test, data.is_training: False}
                v_p = sess.run(fraggi_softmax_out, feed_dict=feed_dict)
                image_x[
                    data.index - data.cur_size : data.index,
                    : data.dim_xyz[0],
                    : data.dim_xyz[1],
            ...
            for ind in range(
                0,
                data.dim_xyz[1] // data.batch_size
                + (data.dim_xyz[1] % data.batch_size > 0),
            ):
            ...
            for ind in range(
                0,
                data.dim_xyz[0] // data.batch_size
                + (data.dim_xyz[0] % data.batch_size > 0),
            ):
            ...
            for index in range(0, data.stack_size):
                ...
                feed_dict = {data.x: x_test, data.m: m_test, data.is_training: False}
                v_p = sess.run(fraggi_softmax_out, feed_dict=feed_dict)
                data.result_image[index] = v_p[
                    0,
                    : data.big_size[0] - data.factor4,
                    : data.big_size[1] - data.factor4,
                    -1,
                ]
    ...
    print(
        "memory used: "
        + str((psutil.virtual_memory()[3] - start_mem) / 10**9)
        + " GB ("
        + str(psutil.virtual_memory()[2] - start_percent)
        + " %)"
    )
    f = open(
        os.path.dirname(os.path.abspath(image).replace("\\", "/")) + "/measurement.csv",
        "a",
    )
    f.write(
        "Memory used (Frangi-Filter);"
        + "{:.3f}".format((psutil.virtual_memory()[3] - start_mem) / 10**9)
        + ";GB\n"
    )
    f.close()
