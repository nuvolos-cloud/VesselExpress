...
class Metrics:
    def __init__(self, path, label_train, label_valid, is_restore, is_test=False):
        ...
        if not is_restore:
            if not is_test:
                ...
                f.write(
                    "Step;Loss Train;Loss Valid;TP;TN;FP;FN;Accuracy;Precision;Sensitivity;Specificity;F1 Score;TP Val;"
        ...
        self.epoch_train = [
            ...
            0,
            0,
        ]  # collects all tp, tn, fp and fn of the whole epoch (training set)
        self.epoch_valid = [0, 0, 0, 0]  # see above but for the validation set
        self.label_train = label_train
        self.label_valid = label_valid
        self.threshold = (
            0.49  # original value of FrangiNet, considering there are only two classes
        )
    ...
    def collect_train_epoch(self, pred_train, batch_images):
        ...
        prep_label = np.zeros(
            shape=(len(batch_images), pred_train.shape[1], pred_train.shape[2]),
            dtype=np.bool,
        )
        index = 0
    ...
    def metrics_to_csv_3d(self, step, loss_total, loss_valid):
        ...
        acc_v, prec_v, sens_v, spec_v, f1_score_v = self.calculate_metrics(
            tp_v, tn_v, fp_v, fn_v
        )
    ...
    def metrics_to_csv(self, step, loss_total, loss_valid, pred_valid, label_valid):
        ...
        tp_v, tn_v, fp_v, fn_v = self.calculate_confusion_matrix(
