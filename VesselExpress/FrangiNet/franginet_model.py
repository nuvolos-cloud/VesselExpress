...
from FrangiNet.franginet_layer import (
    conv2d,
    elementwise2d,
    nonlinear2d,
    elementwise_sum,
...
class FrangiNet:
    ...
    def init_kernels(self):
        ...
        for i in range(len(self.sigma_list)):
            ...
            yy_mat[:, :, 0, 0] = yy2d
            xy_mat[:, :, 0, 0] = xy2d
            w_xx = tf.get_variable(
                initializer=xx_mat, dtype=tf.float32, name="w_xx_%d" % i, trainable=True
            )
            w_yy = tf.get_variable(
                initializer=yy_mat, dtype=tf.float32, name="w_yy_%d" % i, trainable=True
            )
            w_xy = tf.get_variable(
                initializer=xy_mat, dtype=tf.float32, name="w_xy_%d" % i, trainable=True
            )
    ...
    def single_scale(self, patch, scale_index, name=""):
        ...
        b2_4ac = elementwise2d(
            xx_yy_sqr, xy_sqr, "add", coefficient=[1.0, 4.0], name=name + "b2_4ac"
        )
        b2_4ac_sqrt = nonlinear2d(b2_4ac, "square_root", name=name + "b2_4ac_sqrt")
        root1 = elementwise_sum(
            scale_xx,
            scale_yy,
            b2_4ac_sqrt,
        ...
        mask = condition_operator(
            x=[0.0], y=lambda1, op="less", name=name + "less_mask"
        )
        ...
        a = tf.Variable(dtype=tf.float32, initial_value=-2.0)
        ...
        v_before_mask = elementwise2d(
            exp_rb_sqr,
            elementwise2d(exp_rb_sqr, exp_s_sqr, op="multiply"),
            op="subtract",
        )
    ...
    def multi_scale(self, inputs, is_training):
        ...
        vesselness = batch_norm_tensor(
            vesselness, is_training=is_training, scope="final/bn1"
        )
