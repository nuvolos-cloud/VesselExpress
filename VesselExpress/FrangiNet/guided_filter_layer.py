...
def build_lr_can(
    ...
    w1 = tf.get_variable(
        shape=(kernel_size, kernel_size, num_channel, num_feature),
        dtype=tf.float32,
        name="lrn/w1",
    )
    inputs = tf.nn.conv2d(
        input=inputs, filter=w1, strides=strides, padding="SAME", name="lrn/conv1"
    )
    ...
    for i in range(2, 1 + num_layer):
        w_i = tf.get_variable(
            shape=(kernel_size, kernel_size, num_feature, num_feature),
            dtype=tf.float32,
            name="lrn/w%d" % i,
        )
        inputs = tf.nn.atrous_conv2d(
            value=inputs,
            filters=w_i,
            rate=2 ** (i - 1),
            padding="SAME",
            name="lrn/dil_conv%d" % i,
        )
        inputs = batch_norm_tensor(
            inputs, is_training=is_training, scope="lrn/bn%d" % i
        )
        inputs = tf.nn.leaky_relu(inputs, alpha=0.2, name="lrn/l_relu%d" % i)

    w_final0 = tf.get_variable(
        shape=(kernel_size, kernel_size, num_feature, num_feature),
        dtype=tf.float32,
        name="lrn/w_final0",
    )
    ...
    w_final1 = tf.get_variable(
        shape=(1, 1, num_feature, num_channel), dtype=tf.float32, name="lrn/w_final1"
    )
...
def deef_guided_filter_advanced(inputs, guid, is_training, r=1, eps=1e-8):
    def guided_map(
        ...
        w2 = tf.get_variable(
            shape=(kernel_size, kernel_size, num_feature, num_channel),
            dtype=tf.float32,
            name=name + "/w2",
        )
