...
def main_frangi_train(conf_file):
    ...
    metric = Metrics(
        data.path_save, data.binary_image, data.valid_binary_image, data.is_restore
    )
    ...
    if data.mode == "vanilla":
        ...
        loss_sim = tf.losses.mean_squared_error(
            predictions=tf.boolean_mask(up_sample(preprocessed), data.m[:, :, :, -1:]),
            labels=tf.boolean_mask(up_sample(data.x), data.m[:, :, :, -1:]),
        )
        loss_wcr, loss_fc = get_loss(logits, data.y, data.m, data.w)
    ...
    learning_rate_node = tf.train.exponential_decay(
        learning_rate=data.rate_learning,
        global_step=data.step_global,
        decay_steps=data.step_decay,
        decay_rate=data.rate_decay,
        staircase=True,
    )
    train_step = tf.train.AdamOptimizer(learning_rate=learning_rate_node).minimize(
        total_loss, name="ADAM", global_step=data.step_global
    )
    ...
    if data.is_restore:
        ...
        logging.info(
            "Model restored from file: {:}".format(data.path_model % data.step_restore)
        )
    ...
    while total_step <= data.step_overall:
        ...
        if data.mode == "vanilla":
            pred_train, loss_train, _ = sess.run(
                [softmax, loss_fc, train_step], feed_dict=feed_dict
            )
        else:
        ...
        if total_step % data.steps_per_epoch == 0:
            logging.info(
                "Training epoch %s finished, loss: %s",
                str(int(total_step / data.steps_per_epoch)),
                str(loss_train),
            )
        ...
        if total_step % data.steps_per_epoch == 0:
            ...
            for _ in range(0, data.steps_per_validation):
                ...
                pred, crop_x, crop_y, crop_m, crop_w, loss_valid = sess.run(
                    [softmax, data.x, data.y, data.m, data.w, total_loss], feed_dict_bv
                )

                # fill in the prepared validation images for calculating metrics
                if total_step % data.step_summary == 0:
                    pred_valid[
                        index * data.size_batch : index * data.size_batch
                        + data.size_batch,
                        :,
                        :,
                    ] = pred[:, :, :, -1]
                    ...
                    index += 1
            logging.info(
                "Validation epoch %s finished, loss: %s",
                str(int(total_step / data.steps_per_epoch)),
                str(loss_valid),
            )
        ...
        if total_step % data.step_summary == 0:
            ...
            metric.metrics_to_csv(
                int(total_step / data.steps_per_epoch),
                loss_train,
                loss_valid,
                pred_valid,
                label_valid,
        ...
        if total_step % data.step_save == 0:
            ...
            model_path = data.path_save + "model/%d/" % int(
                total_step / data.steps_per_epoch
            )
            ...
            io.imsave(
                model_path + "/label.tif", crop_y[0, :, :, -1], check_contrast=False
            )

            if data.restrict_roc is True:
                if data.create_prc is True:
                    metrics.precision_recall(
                        pred_valid[: data.restrict_amount, :, :],
            ...
            logging.info(
                "Model epoch %s saved and metrics created",
                str(int(total_step / data.steps_per_epoch)),
            )
        ...
        if data.early_stopping is True and total_step % data.steps_per_epoch == 0:
            ...
            if sum(epoch_loss) / 10 < data.delta_loss:
                if total_step % data.step_save == 0:
                    ...
                    pred_log, crop_x, crop_y, crop_m, crop_w = sess.run(
                        parameters, feed_dict_bv
                    )
                    ...
                    logging.info(
                        "Model of epoch %s has been saved"
                        % str(int(total_step / data.steps_per_epoch))
                    )

                logging.info(
                    "Training has stopped early in epoch %s."
                    % str(int(total_step / data.steps_per_epoch))
                )
                break
