...
class DataProvider:
    ...
    def init_image(self):
                ...
                if self.rotate_images is True:
                    ...
                    if self.image is not None:
                        self.image = np.concatenate(
                            (self.image, temp_net_image), axis=0
                        )
                        self.binary_image = np.concatenate(
                            (self.binary_image, temp_net_label), axis=0
                        )
                        if self.use_mask is True:
                            self.mask_image = np.concatenate(
                                (self.mask_image, temp_net_mask), axis=0
                            )
                    ...
                    if self.image is not None:
                        ...
                        self.binary_image = np.concatenate(
                            (self.binary_image, net_label), axis=0
                        )
                        if self.use_mask is True:
                            self.mask_image = np.concatenate(
                                (self.mask_image, net_mask), axis=0
                            )
                    else:
        ...
        self.result_image = np.zeros(
            shape=(self.image.shape[0], self.image_big[0], self.image_big[1]),
            dtype=np.float32,
        )
    ...
    def provide_test(self):
        ...
        """
        x = np.zeros(
            shape=(1, self.image_small[0], self.image_small[1], self.num_channel),
            dtype=np.float32,
        )
        x[:, :, :, 0] = self.image[self.index]

        m = np.ones(
            shape=(1, self.image_big[0], self.image_big[1], self.num_class),
            dtype=np.bool,
        )
        m[:, :, :, 1:] = m[:, :, :, 0:1]
