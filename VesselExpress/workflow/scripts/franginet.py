...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Use FrangiNet to filter on image files of type .tif"
    )
    ...
    parser.add_argument(
        "-average",
        type=str,
        help="if true the average of 3D is taken, else the maximum",
    )
    parser.add_argument(
        "-mode_img", type=str, help="mode how the image is being processed"
    )
    parser.add_argument("-gpus", type=str, help="number of the gpus that shall be used")
    parser.add_argument(
        "-batch_size",
        type=str,
        help="only mode OneCubeBatch: size of the batch to be processed",
    )
    ...
    f = open(
        os.path.dirname(os.path.abspath(args.i).replace("\\", "/"))
        + "/measurement.csv",
        "a",
    )
