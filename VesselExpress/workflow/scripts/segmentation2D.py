...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Computes Li Thresholding on image file of type .jpg"
    )
    parser.add_argument("-i", type=str, help="input jpg image file to process")
    parser.add_argument(
        "-sigma_min", type=float, default=1, help="Frangi sigma_min parameter"
    )
    parser.add_argument(
        "-sigma_max", type=float, default=10, help="Frangi sigma_max parameter"
    )
    parser.add_argument(
        "-sigma_steps", type=float, default=2, help="Frangi sigma_steps parameter"
    )
    parser.add_argument(
        "-alpha", type=float, default=0.5, help="Frangi alpha parameter"
    )
    ...
    parser.add_argument(
        "-denoise", type=int, default=0, help="set to 1 for prior denoising of image"
    )
    parser.add_argument(
        "-back_sub",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=137,
        help="block size for local thresholding, " "0 for global thresholding",
    )
    ...
    filtered = frangi(
        image=img,
        black_ridges=False,
        sigmas=np.arange(args.sigma_min, args.sigma_max, args.sigma_steps),
        alpha=args.alpha,
        beta=args.beta,
        gamma=args.gamma,
    )
    ...
    # Li thresholding
        ...
        if args.block_size == 0:
            ...
            threshold = threshold_local(
                filtered, block_size=args.block_size
            )  # local thresholding
