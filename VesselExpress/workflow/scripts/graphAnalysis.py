...
def processImage(skelImg, binImg, parameterDict):
    ...
    stats = graph.Graph(
        ...
        skeleton,
        networkxGraph,
        parameterDict.get("pixel_dimensions"),
        pruningScale=parameterDict.get("pruning_scale"),
        lengthLimit=parameterDict.get("length_limit"),
        diaScale=parameterDict.get("dia_scale"),
        branchingThreshold=parameterDict.get("branching_threshold"),
        expFlag=parameterDict.get("experimental_flag"),
        infoFile=finfo,
        graphCreation=parameterDict.get("extended_output"),
        smallRAMmode=parameterDict.get("small_RAM_mode"),
        fileName=file_name,
        removeBorderEndPts=parameterDict.get("remove_border_end_pts"),
        removeEndPtsFromSmallFilaments=parameterDict.get(
            "remove_end_pts_from_small_filaments"
        ),
        interpolate=parameterDict.get("seg_interpolate"),
        splineDegree=parameterDict.get("spline_degree"),
    ...
    if parameterDict.get("extended_output") == 1:
        ...
        utils.write_img(
            (graph_arr * 255).astype("uint8"),
            dir + "/Graph_" + file_name + "." + input_file.split(".")[1],
        )
        ...
        for ind in brPts:
            brPt_img[ind] = 255
        utils.write_img(
            brPt_img.astype("uint8"),
            dir + "/BrPts_" + file_name + "." + input_file.split(".")[1],
        )
        ...
        for ind in endPts:
            endPt_img[ind] = 255
        utils.write_img(
            endPt_img.astype("uint8"),
            dir + "/EndPts_" + file_name + "." + input_file.split(".")[1],
        )
    ...
    utils.saveAllStatsAsCSV(
        stats.segStatsDict,
        dir + "." + input_file.split(".")[1] + "_Segment_Statistics.csv",
        file_name,
    )
    utils.saveAllFilStatsAsCSV(
        stats.filStatsDict,
        dir + "." + input_file.split(".")[1] + "_Filament_Statistics.csv",
        file_name,
    )
    utils.saveBranchesBrPtAsCSV(
        stats.branchesBrPtDict,
        dir + "." + input_file.split(".")[1] + "_BranchesPerBranchPt.csv",
        file_name,
    )
    if parameterDict.get("experimental_flag") == 1:
        statsDir = os.path.join(dir, "statistics")
        os.makedirs(statsDir, exist_ok=True)
        utils.saveSegmentDictAsCSV(
            stats.segStatsDict,
            os.path.join(statsDir, file_name + "_Segment_z_Angle.csv"),
            "Segment z Angle",
            "zAngle",
            "°",
        )
        utils.saveEndPtsRelativeAsCSV(
            stats.endPtsTopVsBottom, dir + "_EndPtsRatio.csv", file_name
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Computes graph analysis on skeleton image file of type .tif"
    )
    parser.add_argument(
        "-skel_img", type=str, help="input skeleton image file to process"
    )
    parser.add_argument("-bin_img", type=str, help="input binary image file to process")
    parser.add_argument(
        "-pixel_dimensions",
        type=str,
        default="2.0,1.015625,1.015625",
    ...
    parser.add_argument(
        "-length_limit", type=float, default=3, help="Limit of vessel lengths"
    )
    parser.add_argument(
        "-dia_scale",
        type=float,
        default=2,
        help="Segments with lengths shorter than their diameter "
    ...
    parser.add_argument(
        "-remove_border_end_pts",
        type=int,
        default=0,
        help="set to 1 to remove terminal points at the " "volume border",
    )
    parser.add_argument(
        "-remove_end_pts_from_small_filaments",
        type=int,
    ...
    parser.add_argument(
        "-seg_interpolate", type=int, default=0, help="set to 1 to interpolate segments"
    )
    parser.add_argument(
        "-spline_degree",
        type=int,
        default=3,
        help="degree of b-spline for segment interpolation",
    )
    parser.add_argument(
        "-cut_neighbor_brpt_segs",
        type=int,
        default=1,
        help="set to 0 to not cut segments which "
        "consists of 2 neighboring branch points",
    )
    parser.add_argument(
        "-small_RAM_mode", type=int, default=0, help="set to 1 for small RAM mode"
    )
    parser.add_argument(
        "-prints", type=bool, default=False, help="set to True to print runtime"
    )
    ...
    if args.prints:
        print(
            "Graph extraction and statistical analysis completed in %0.3f seconds"
            % (time.time() - programStart)
        )
