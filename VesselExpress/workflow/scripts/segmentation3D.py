...
def vesselness_filter(
    ...
    if dim == 3:
        ...
        hessian_itk = itk.hessian_recursive_gaussian_image_filter(
            im_itk, sigma=sigma, normalize_across_scale=True
        )
        vess_tubulness = itk.hessian_to_objectness_measure_image_filter(
            hessian_itk, object_dimension=1, gamma=gamma
        )
...
class vesselness_filter_dask:
    ...
    def compute_vesselness(self, im_smooth):
        return vesselness_filter(
            im_smooth.astype(float), sigma=self.sigma, gamma=self.gamma
        )
...
def dask_threshold(im, cutoff_method):
    ...
    threshold_in_chunks = im.map_blocks(
        threshold_function.calculate_by_chunks, chunks=(1, 1, 1), dtype="float"
    ).compute()
...
def segmentation(input, output, cfg):
    if cfg["small_RAM_mode"] == 1:
        ...
        cutoff_value = ndmeasure.mean(im_smooth) + cfg[
            "core_threshold"
        ] * ndmeasure.standard_deviation(im_smooth)
        ...
        if cfg["core_vessel_1"] == 1:
            vess_func = vesselness_filter_dask(
                sigma=cfg["sigma_1"], gamma=cfg["gamma_1"]
            )
            ...
            th_1 = dask_threshold(vess_1, cutoff_method=cfg["cutoff_method_1"])
            seg_1 = vess_1 > th_1
        ...
        if cfg["core_vessel_2"] == 1:
            ...
            vess_func = vesselness_filter_dask(
                sigma=cfg["sigma_2"], gamma=cfg["gamma_2"]
            )
            ...
            th_2 = dask_threshold(vess_2, cutoff_method=cfg["cutoff_method_2"])
            seg_2 = vess_2 > th_2
        ...
        if cfg["post_closing"] != 0:
            ...
            seg_refined = ndmorph.binary_closing(
                seg_input, structure=cube(cfg["post_closing"]), border_value=1
            )
        ...
        if cfg["post_thinning"] == 1:
            ...
            seg_refined = da.map_overlap(
                thin_func, seg_input, dtype="bool", depth=3, boundary="reflect"
            )
        ...
        if cfg["post_cleaning"] != 0:
            ...
            seg_refined = da.map_overlap(
                clean_func, seg_input, dtype="bool", depth=3, boundary="reflect"
            )
        ...
        if cfg["core_vessel_1"] == 1:
            out = vesselness_filter(
                im, cfg["sigma_1"], cfg["gamma_1"], cfg["cutoff_method_1"]
            )
            seg = np.logical_or(seg, out)

        if cfg["core_vessel_2"] == 1:
            out = vesselness_filter(
                im, cfg["sigma_2"], cfg["gamma_2"], cfg["cutoff_method_2"]
            )
            seg = np.logical_or(seg, out)
        ...
        if cfg["post_thinning"] == 1:
            seg = topology_preserving_thinning(
                seg, min_thickness=cfg["min_thickness"], thin=cfg["thin"]
            )
        if cfg["post_cleaning"] != 0:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="segmentation of 3D images of type .tif"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="set to 1 for processing files larger than RAM",
    )
    parser.add_argument(
        "-smoothing",
        type=int,
        default=1,
        help="set to 1 for smoothing, 0 for no smoothing",
    )
    parser.add_argument(
        "-core_threshold",
        type=float,
        default=3.0,
        help="set to 0 for no core-threshold",
    )
    parser.add_argument(
        "-core_vessel_1",
        type=int,
        default=1,
        help="set to 1 for first vesselness filter, " "0 for no vesselness filter",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="set to 1 for second vesselness filter, " "0 for no vesselness filter",
    )
    ...
    parser.add_argument(
        "-post_closing", type=int, default=5, help="set to 0 for no closing"
    )
    parser.add_argument(
        "-post_thinning",
        type=int,
        default=0,
        help="set to 1 for post-processing, " "0 for no post-processing",
    )
    parser.add_argument("-min_thickness", type=int, default=None)
    parser.add_argument("-thin", type=int, default=None)
    parser.add_argument(
        "-post_cleaning", type=int, default=100, help="set to 0 for no cleaning"
    )
