...
def render_object(
    ...
    save_glb: bool = True,
    # Camera Params
    camera_pos_x: float = -2.0,
    camera_pos_y: float = 3.0,
    camera_pos_z: float = 3.0,
    camera_euler_angle_x: float = 422.0,
    camera_euler_angle_y: float = 0.0,
    camera_euler_angle_z: float = 149.0,
    # Background Color Params
    background_transparent: bool = False,
    background_r: float = 0.638,
    background_g: float = 0.638,
    background_b: float = 0.638,
    ...
    mesh_roughness: float = 0.65,
    mesh_metallic: float = 0.0,
    mesh_sheen: float = 0.0,
    mesh_specular: float = 0.0,
    # Render & Hardware Params
    render_engine: str = "CYCLES",
    render_device="GPU",
    render_distance: int = 500000,
    # Rendered Image Params
    image_resolution_x: int = 1920,
    image_resolution_y: int = 1080,
    image_resolution_scale: float = 50.0,
    ...
    if render_engine not in expected_render_engine:
        raise Exception(
            'Invalid parameter for "render_engine". Got: '
            + render_engine
            + ". Expected: "
            + str(expected_render_engine)
        )
    if render_device not in expected_render_device:
        raise Exception(
            'Invalid parameter for "render_device". Got: '
            + render_device
            + ". Expected: "
            + str(expected_render_device)
        )
    if image_color_mode not in expected_image_color_mode:
        raise Exception(
            'Invalid parameter for "image_color_mode". Got: '
            + image_color_mode
            + ". Expected: "
            + str(expected_image_color_mode)
        )
    if file_format not in expected_file_format:
        raise Exception(
            'Invalid parameter for "file_format". Got: '
            + file_format
            + ". Expected: "
    ...
    if image_bit_depth not in expected_image_bit_depth:
        raise Exception(
            'Invalid parameter for "image_bit_depth". Got: '
            + str(image_bit_depth)
            + ". Expected: "
            + str(expected_image_bit_depth)
        )
    ...
    camera.rotation_euler = [
        radians(a)
        for a in (camera_euler_angle_x, camera_euler_angle_y, camera_euler_angle_z)
    ]
    ...
    bpy.data.worlds["World"].node_tree.nodes["Background"].inputs[
        1
    ].default_value = background_intensity
    ...
    bpy.data.materials[mat_name].node_tree.nodes["Principled BSDF"].inputs[
        0
    ].default_value = [mesh_r, mesh_g, mesh_b, mesh_a]
    bpy.data.materials[mat_name].node_tree.nodes["Principled BSDF"].inputs[
        4
    ].default_value = mesh_metallic
    bpy.data.materials[mat_name].node_tree.nodes["Principled BSDF"].inputs[
        5
    ].default_value = mesh_specular
    bpy.data.materials[mat_name].node_tree.nodes["Principled BSDF"].inputs[
        7
    ].default_value = mesh_roughness
    bpy.data.materials[mat_name].node_tree.nodes["Principled BSDF"].inputs[
        10
    ].default_value = mesh_sheen
    ...
    if save_glb:
        ...
        bpy.ops.export_scene.gltf(
            filepath=glb_out_file_name,
            export_copyright="Created with VesselExpress",
...
if __name__ == "__main__":
    if "--" in sys.argv:
        ...
        parser.add_argument(
            ...
            required=True,
            type=str,
            help="Path to the directory to save the renders in.",
        )
        ...
        parser.add_argument(
            ...
            type=int,
            default=1,
            help="Save the .blend project file after rendering.",
        )
        parser.add_argument(
            "-save_glb",
            type=int,
            default=1,
            help="Save the .glb object file after rendering.",
        )
        ...
        parser.add_argument(
            ...
            type=float,
            default=-2.0,
            help="Original camera x-position.",
        )
        parser.add_argument(
            "-camera_pos_y", type=float, default=3.0, help="Original camera y-position."
        )
        parser.add_argument(
            "-camera_pos_z", type=float, default=3.0, help="Original camera z-position."
        )
        ...
        parser.add_argument(
            ...
            type=float,
            default=422.0,
            help="Original camera x-euler-angle.",
        )
        parser.add_argument(
            "-camera_angle_y",
            type=float,
            default=0.0,
            help="Original camera y-euler-angle.",
        )
        parser.add_argument(
            "-camera_angle_z",
            type=float,
            default=149.0,
            help="Original camera z-euler-angle.",
        )
        ...
        parser.add_argument(
            ...
            type=int,
            default=0,
            help="If true, the background will be rendered transparently. Should be used in combination with a file format that support alpha transparency and CYCLES renderer.",
        )
        parser.add_argument(
            "-background_r",
            type=float,
        ...
        parser.add_argument(
            ...
            type=float,
            default=1.0,
            help="Background color intensity.",
        )
        ...
        parser.add_argument(
            ...
            type=str,
            default="CYCLES",
            help="Rendering engine chosen. Expected Values: "
            + str(expected_render_engine),
        )
        parser.add_argument(
            "-render_device",
            type=str,
            default="GPU",
            help="Hardware device to render. Expected Values: "
            + str(expected_render_device),
        )
        parser.add_argument(
            "-render_distance",
        ...
        parser.add_argument(
            ...
            type=int,
            default=960,
            help="Output image pixel resolution: x",
        )
        parser.add_argument(
            "-image_resolution_y",
            type=int,
            default=540,
            help="Output image pixel resolution: y",
        )
        parser.add_argument(
        ...
        parser.add_argument(
            ...
            type=str,
            default="RGBA",
            help="Output image color mode. Expected values: "
            + str(expected_image_color_mode),
        )
        parser.add_argument(
            "-image_bit_depth",
            type=int,
            default=8,
            help="Output image bit depth. Expected values: "
            + str(expected_image_bit_depth),
        )
        parser.add_argument(
            "-file_format",
            type=str,
            default="PNG",
            help="Output image file format. Expected values: "
            + str(expected_file_format),
        )
        parser.add_argument(
            "-image_compression",
        ...
        print(
            "Not enough arguments supplied. Run this function and provide the necessary arguments."
        )
